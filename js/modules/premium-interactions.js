...
if (!isMobileDevice) {
  ...
  if (workList && cards.length > 0 && worksEl) {
    ...
    workList.style.transform = `translateX(-30vw) translateY(140px) rotateY(${baseY}deg) rotateX(${baseX}deg) rotateZ(${baseZ}deg)`;
    ...
    function animateHover() {
      ...
      if (Math.abs(diffY) > 0.001 || Math.abs(diffX) > 0.001 || Math.abs(diffZ) > 0.001) {
        ...
        workList.style.transform = `translateX(-30vw) translateY(140px) rotateY(${currentWorkListY}deg) rotateX(${currentWorkListX}deg) rotateZ(${currentWorkListZ}deg)`;
      }
