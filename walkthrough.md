...
## 🛠️ Feature: 预览悬浮窗 3D 偏斜面与作品卡片 3D 面共面对齐优化

### 1. 方案设计与视觉意图
- **三维共面浮动感 (Aligned 3D Co-planar Floating)**:
  - 现象：原本的悬浮预览视口在静止时是平行于屏幕二维平面的（`rotateX: 0, rotateY: 0, rotateZ: 0`），只有在移动时才产生物理阻尼偏斜。而下方的作品列表 `.work-list` 整体是在三维空间中呈现偏斜状态的（`rotateY(-34deg) rotateX(17deg) rotateZ(2deg)`）。这使得预览窗与它所对应的卡片之间缺乏三维空间上的平行对齐感，显得像两个独立空间的层叠加。
  - **创意设想**：让预览图和橙色背景色块**默认就以和作品列表完全一致的 3D 偏斜角度（`baseY = -34`, `baseX = 17`, `baseZ = 2`）浮现在屏幕上**，仿佛它们是在与作品列表同一个三维斜面上平行悬浮飞行。当鼠标移动时，鼠标的动态加速度在这套“基准 3D 斜面”之上进行叠加偏斜，创造出一种震撼、连贯、且极具物理空间实感的高端三维视觉享受。

### 2. 解决方案与修改
- **基准 3D 旋转角度对齐 (Base 3D Angle Alignment)**:
  - 修改 [premium-interactions.js](file:///C:/Users/jackchen/lobsterai/project/Project-C/portfolio-v3/js/modules/premium-interactions.js#L381-L390) 里的鼠标首帧进入初始化逻辑 `firstMove`，将 `currentTiltX/Y/Z` 和 `currentOrangeTiltX/Y/Z` 的初始值从 `0` 修改为卡片列表的基准偏斜值 `baseX`, `baseY`, `baseZ`。保证淡入时直接以正确倾角入场，不发生闪烁突变。
- **加速度叠加公式改造**:
  - 重构图片层与背景层的目标角度计算：
    - **图片层 (imgContainer)**:
      `targetTiltY = baseY + clamp(-16, 16, dx * 0.06)`
      `targetTiltX = baseX + clamp(-16, 16, -dy * 0.06)`
      `targetTiltZ = baseZ + clamp(-6, 6, dx * 0.02)`
    - **背景橙色层 (orangeLayer)**:
      为维持其原本的高级视差阻尼感，我们将鼠标加速度产生的动态偏移量乘以 `0.8`，但**保持基准旋转角 `baseX/Y/Z` 100% 对齐不缩放**，使得它们静止时完全平行，运动时产生灵动的视差：
      `targetOrangeTiltY = baseY + clamp(-16, 16, dx * 0.06) * 0.8`
      `targetOrangeTiltX = baseX + clamp(-16, 16, -dy * 0.06) * 0.8`
      `targetOrangeTiltZ = baseZ + clamp(-6, 6, dx * 0.02) * 0.8`
  - 并在 `gsap.set(orangeLayer)` 时直接应用 `currentOrangeTiltX/Y/Z`，消去多余的全局比例折算。
