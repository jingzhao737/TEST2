...
3. **Subpixel Snapping & Initial Coordinate Snapping (Zero Drift & Zero Warp)**:
   - **Static Snap**: Added active snapping checks in the animation loop. When both mouse speed and cursor speed fall below `0.05px/frame`, the rotation angles and pitches snap exactly to their resting values (`currentAngle = -90`, `currentRoll = 0`, `spinRoll = 0`). This guarantees that when the mouse comes to a halt, the flight wing is perfectly flat, upright, and symmetrical with zero residual subpixel drift.
   - **Coordinate Snap on Entry**: Introduced an `isFirstMove` flag. When the page first loads or when the mouse re-enters the window after a `mouseleave` event, the very first `mousemove` event instantly snaps the custom cursor's position coordinates (`cX`, `cY`, `t1X`, `t1Y`) and velocity buffers to the current mouse position. This completely prevents the cursor from "warp-flying" across the screen from `(0, 0)` or the exit point to the entry point, eliminating the violent, momentary stretching/twitching on entry.
