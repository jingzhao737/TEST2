...
1. **Snappy Flat Spin Trigger (Spring-decayed Z-rotation)**:
   - When the cursor enters a new interactive hover target, it triggers a very subtle, instantaneous **rotational click kick** (`-25` degrees) around its Z-axis.
   - The kick snaps back in ~200ms with a clean spring bounce (`tension: 0.35`, `damping: 0.48`), creating a tactile "tick" physical feedback that aligns with the visual hover event without being distracting or exaggerated.
...
2. **Torsional Wing-Sweep (Multi-layered Twist)**:
   ...
   - The lower layers fan out progressively in alternating directions to remain **perfectly balanced and symmetrical** (net rotation offset is `0`), scaled down to a highly restrained, subtle fanning spread (maximum `6` degrees):
     - `Layer 3 (mid-1)`: `-3deg`
     - `Layer 2 (mid-2)`: `+3deg`
     - `Layer 1 (mid-3)`: `-6deg`
     - `Layer 0 (back)`: `+6deg`
   - This creates a gorgeous, subtle iridescent geometric prism structure on hover while maintaining a centered visual balance.
...
4. **3D Extrusion Depth Expansion**:
   - The dynamic Z-depth spacing between the 5 stacked layers expands from `1.0px` to a very subtle `2.2px` on hover.
   - With the cursor container's `perspective` set at `45px` in [styles.css](file:///D:/webprojext/styles.css), this creates a refined 3D thickness expansion, making the crystal look three-dimensional and solid without looking over-stretched or distorted.

5. **Tuned Scale Integration**:
   - The hover scale factor is tuned to a very moderate `0.74` (compared to the baseline `0.67`). The minimal scale-up preserves absolute restraint, letting the subtle 3D fanning and depth expansion serve as the primary interactive reactions.
