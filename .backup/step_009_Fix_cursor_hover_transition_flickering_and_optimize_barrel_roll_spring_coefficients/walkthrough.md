...
1. **Snappy 3D Barrel Roll Trigger (Spring-decayed Y-rotation)**:
   ...
   - The rotation is driven by spring physics (`tension: 0.12`, `damping: 0.84`), allowing it to spin with high velocity, overshoot the target angle slightly, and bounce back into a steady orientation, creating an organic, satisfying tactile feel.
...
2. **Torsional Wing-Sweep (Multi-layered Twist)**:
   ...
   - The lower layers fan out progressively in alternating directions:
     - `Layer 3 (mid-1)`: `-6deg`
     - `Layer 2 (mid-2)`: `+12deg`
     - `Layer 1 (mid-3)`: `-18deg`
     - `Layer 0 (back)`: `+24deg`
   - This creates a gorgeous iridescent geometric prism structure on hover, showcasing the individual layered colors.

3. **3D Extrusion Depth Expansion**:
