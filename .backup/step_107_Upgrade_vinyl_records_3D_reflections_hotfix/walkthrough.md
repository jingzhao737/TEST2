...
## 🛠️ Feature: 唱片拟真 3D 质感与交互光影升级 (Skeuomorphic 3D Vinyl Disc Upgrade)

### 1. 需求分析与修改
- **问题反馈**：唱片视觉效果过于扁平平直，缺乏立体的厚度感、凹槽质感与逼真的反光光影。
- **解决方案与重构**：
  - **动态 3D 悬浮投影 (Dynamic Elevation Shadow)**：增加独立阴影绘制层。阴影偏移量与模糊半径会根据拖拽状态和 Hover 状态发生 LERP 缓动，实现拖起盘面时“高差增大、阴影淡出”的高空物理层级。
  - **双锥形径向金属高光 (Hourglass Specular Shine)**：重构了反射光渲染，在唱片表面增加了高保真的双锥体金属偏振镜面反光。通过获取鼠标实时坐标，实现高光轴向跟随鼠标微幅偏转的交互动效。
  - **独立层绘制 (Separated Layer Rendering)**：
    - 将盘面图片、黑白细密凹槽（Grooves）以及中心 spindle 孔绑定旋转；
    - 将环境高光反射、3D Bevel 边缘（顶左亮线、底右暗线）锁定为静态不旋转。两者叠加，呈现出物理上极其真实的“唱片在旋转但光影保持稳定”的高级反光视觉。
...
## 🛠️ Hotfix: 优化唱片 3D 光影质感，去除硬边阴影与生硬反光面

### 1. 需求分析与修改
- **问题反馈**：第一版 3D 效果显得有些劣质和生硬（如阴影带有生硬轮廓、反光呈锋利扇形楔片）。
- **解决方案与重构**：
  - **柔和径向渐变阴影 (Soft Radial Gradient Shadow)**：用纯 canvas 径向渐变代替 ctx.shadowColor 搭配 solid fill 的做法，使悬浮阴影自然由暗向边缘晕染消散，彻底消除任何硬边界硬圈。
  - **羽化交错光芒 (Crossed Feathered Linear Sheen)**：废除了生硬的 ctx.arc() 扇形楔片反光面，改为绘制两条夹角约 72 度的、带羽化宽度的线性渐变光芒 (Feathered Sheen Bar)，高光边缘呈现完美的渐隐，完美还原物理偏振折射的质感。
  - **多磁道凹槽与内凹印记 (Multi-track & Inset Label)**：
    - 将同心圆槽线划分为 3 个磁道区段，中途留有平滑镜面的音轨隔离带，符合真实黑胶灌录排版；
    - 在中心标签边界增加了向内收缩的径向渐变暗角 (Inset Shadow)，逼真再现中心纸贴纸“压印陷入”胶盘的立体边缘。
  - **彩色半透黑胶材质 (Colored Translucent Vinyl)**：微调黑胶遮罩透明度，使底层唱片封面的图案和色调隐约透出，呈现更显昂贵的高级彩色半透黑胶质感。
