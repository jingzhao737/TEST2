...
(function() {
  ...
  (function loop() {
    ...
    // Dynamic Z-depth expansion LERP (spacing goes from 1px to 2.8px on hover, expanding 3D crystal thickness)
    const targetZSpacing = isHovered ? 2.8 : 1.0;
    currentZSpacing += (targetZSpacing - currentZSpacing) * 0.15;

    // Apply Z-translates on the 5 stacked 3D layers to create dynamic extrusion expansion
    if (cursorLayers) {
